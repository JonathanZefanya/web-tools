...
<form name="update_biolink_" method="post" role="form" data-type="<?= $row->type ?>" enctype="multipart/form-data">
    ...
    <div class="form-group">
        <label for="header_background_type_<?= $row->biolink_block_id ?>"><i class="fas fa-fw fa-sm fa-images text-muted mr-1"></i> <?= l('biolink_header.background_type') ?></label>
    ...
    <div data-background-type="image" class="form-group" data-file-image-input-wrapper data-file-input-wrapper-size-limit="<?= settings()->links->background_size_limit ?>" data-file-input-wrapper-size-limit-error="<?= sprintf(l('global.error_message.file_size_limit'), settings()->links->background_size_limit) ?>">
        <label for="<?= 'header_background_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-image fa-sm text-muted mr-1"></i> <?= l('biolink_header.background') ?></label>
        <?= include_view(THEME_PATH . 'views/partials/custom_file_image_input.php', [
            'id'=> 'header_background_' . $row->biolink_block_id,
            'uploads_file_key' => 'backgrounds',
            'file_key' => 'background',
            'already_existing_image' => $row->settings->background,
            'image_container' => 'background',
            'accept' => \Altum\Uploads::array_to_list_format($data->biolink_blocks['header']['whitelisted_image_extensions']),
    ...
    <div data-background-type="image" class="form-group">
        <label for="<?= 'header_background_alt_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-comment-dots fa-sm text-muted mr-1"></i> <?= l('biolink_link.image_alt') ?></label>
        <input id="<?= 'header_background_alt_' . $row->biolink_block_id ?>" type="text" class="form-control" name="background_alt" value="<?= $row->settings->background_alt ?>" maxlength="100" />
        <small class="form-text text-muted"><?= l('biolink_link.image_alt_help') ?></small>
    </div>

    <div data-background-type="video" class="form-group">
        <label for="<?= 'header_video_url_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-video fa-sm text-muted mr-1"></i> <?= l('biolink_header.video_url') ?></label>
        <input id="<?= 'header_video_url_' . $row->biolink_block_id ?>" type="text" class="form-control" name="video_url" value="<?= $row->settings->video_url ?>" maxlength="2048" placeholder="<?= l('biolink_header.video_url_placeholder') ?>" required="required" />
    </div>
    ...
    <div class="form-group" data-file-image-input-wrapper data-file-input-wrapper-size-limit="<?= settings()->links->avatar_size_limit ?>" data-file-input-wrapper-size-limit-error="<?= sprintf(l('global.error_message.file_size_limit'), settings()->links->avatar_size_limit) ?>">
        <label for="<?= 'header_avatar_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-image fa-sm text-muted mr-1"></i> <?= l('biolink_header.avatar') ?></label>
        <?= include_view(THEME_PATH . 'views/partials/custom_file_image_input.php', [
            'id'=> 'header_avatar_' . $row->biolink_block_id,
    ...
    <div class="form-group">
        <label for="<?= 'header_avatar_alt_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-comment-dots fa-sm text-muted mr-1"></i> <?= l('biolink_link.image_alt') ?></label>
        <input id="<?= 'header_avatar_alt_' . $row->biolink_block_id ?>" type="text" class="form-control" name="avatar_alt" value="<?= $row->settings->avatar_alt ?>" maxlength="100" />
        <small class="form-text text-muted"><?= l('biolink_link.image_alt_help') ?></small>
    </div>

    <div class="form-group">
        <label for="<?= 'header_avatar_size_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-expand fa-sm text-muted mr-1"></i> <?= l('biolink_avatar.size') ?></label>
        <select id="<?= 'header_avatar_size_' . $row->biolink_block_id ?>" name="avatar_size" class="custom-select">
            ...
            <option value="125" <?= $row->settings->avatar_size == '125' ? 'selected="selected"' : null ?>>125x125px</option>
        </select>
    </div>
    ...
    <div class="form-group">
        <label for="<?= 'block_location_url_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-link fa-sm text-muted mr-1"></i> <?= l('biolink_link.location_url') ?></label>
        <input id="<?= 'block_location_url_' . $row->biolink_block_id ?>" type="text" class="form-control" name="location_url" value="<?= $row->location_url ?>" maxlength="2048" placeholder="<?= l('global.url_placeholder') ?>" />
    </div>

    <div class="form-group custom-control custom-switch">
        <input
                id="<?= 'block_open_in_new_tab_' . $row->biolink_block_id ?>"
                name="open_in_new_tab" type="checkbox"
                class="custom-control-input"
            <?= $row->settings->open_in_new_tab ? 'checked="checked"' : null ?>
        >
        <label class="custom-control-label" for="<?= 'block_open_in_new_tab_' . $row->biolink_block_id ?>"><?= l('biolink_link.open_in_new_tab') ?></label>
    </div>
...
<?php ob_start() ?>
    <script>
        ...
        type_handler('form[name="update_biolink_"] input[name="background_type"]', 'data-background-type');
        document.querySelector('form[name="update_biolink_"] input[name="background_type"]') && document.querySelectorAll('form[name="update_biolink_"] input[name="background_type"]').forEach(element => element.addEventListener('change', () => { type_handler('form[name="update_biolink_"] input[name="background_type"]', 'data-background-type');}));
    </script>
